...
def get_word_embeddings(df_column, b, n):
    ...
    for i in range(b,n):
        for key in words:
            if key == df_column['all_triggers'].iloc[i]:
                ...
                dicts={id:dic}
                word_embedding_dicts.update(dicts)
    with open('word_embeddings_smell_related00.json', 'w') as fp:
        json.dump(word_embedding_dicts, fp, indent=4)
    return word_embedding_dicts
    # return word_embedding_dicts
