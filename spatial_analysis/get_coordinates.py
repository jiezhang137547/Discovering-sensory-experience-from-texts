...
df = pd.read_excel("tweets_smell_LOC_after_trainedNER_modified_reduced318.xlsx")
print(len(df))
...
for i in range(0, len(location), 1):
    ...
    try:
        ...
        tweets.append(tweet)
        processed_=df['processed'].iloc[i]
        ...
        p=df['place'].iloc[i]
        place.append(p)
        potential_location=df['potential_locations'].iloc[i]
...
dicts={"location":addresses, "LOC":LOC, "place":place, "potential_locations":potential_locations, "lon":lons,"lat":lats, "id":id,"tweet":tweets,"processed":processed,"triggers":index}
...
locations.to_csv('LOC_triggers_locations_317.csv',index=False)
